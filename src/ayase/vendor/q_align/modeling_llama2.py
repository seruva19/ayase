...
import transformers
# vendored compat: transformers >=4.48 no longer exposes many of the symbols the
# upstream `from ...modeling_llama import *` relied on. Import the still-existing
# high-level classes explicitly, and take the removed / version-sensitive Llama
# primitives from the in-tree ._llama_compat (vendored verbatim from v4.36.2, whose
# RoPE convention matches the self-defined attention below).
...
from transformers.cache_utils import Cache, DynamicCache  # vendored compat: upstream used Cache without importing it
from transformers.modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast  # vendored compat: no longer re-exported by modeling_llama *

from torch.nn import CrossEntropyLoss  # vendored compat: no longer provided via import *

from ._llama_compat import (  # vendored compat: removed from modeling_llama in >=4.48
    LlamaRMSNorm,
...
# vendored compat: flash-attention helpers are only needed when the flash_attention_2
# path actually runs; import them defensively so the module still loads without flash_attn.
try:
...
logger = logging.get_logger(__name__)  # vendored compat: no longer provided via import *
...
def model_forward(
    ...
    # vendored compat: transformers >=4.41 removed the `_use_flash_attention_2` /
    # `_use_sdpa` model attributes; derive the mask handling from config._attn_implementation.
