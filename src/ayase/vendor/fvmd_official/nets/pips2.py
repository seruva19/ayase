# Modified for Ayase: imports target the vendored package namespace; model
# architecture and inference math remain the upstream FVMD implementation.
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from functools import partial
from ..utils import basic, misc, samp
...
class Pips(nn.Module):
    ...
    def forward(self, trajs_e0, rgbs, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, feat_init=None, is_train=False, beautify=False):
        ...
        if feat_init is not None:
            ...
            feat1 = samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B,N,C
        ...
        for itr in range(iters):
            ...
            if itr >= 1:
                ...
                feats2_ = samp.bilinear_sample2d(fmaps2_, coords2_[:,:,0], coords2_[:,:,1]).permute(0, 2, 1) # B*S, N, C
                feats2 = feats2_.reshape(B,S,N,self.latent_dim)
                feats4_ = samp.bilinear_sample2d(fmaps4_, coords4_[:,:,0], coords4_[:,:,1]).permute(0, 2, 1) # B*S, N, C
