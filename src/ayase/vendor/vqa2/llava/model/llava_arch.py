...
from .slowfast.builder import slowfast
from .slowfast_projector.slowfast_projector import slowfast_projector
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.slowfast_projector = slowfast_projector()
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            split_sizes = [image.shape[0] for image in images_list][1]
            encoded_image_features,encoded_slowfast_features = self.encode_images(images)
            encoded_image_features.cuda()
            encoded_slowfast_features.cuda()
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if self.config.mm_newline_position == "grid":
                            ...
                            # image_feature=torch.cat((image_feature, encoded_slowfast_features), dim=0)

                            new_image_features.append(image_feature)
                            new_image_features.append(encoded_slowfast_features)
