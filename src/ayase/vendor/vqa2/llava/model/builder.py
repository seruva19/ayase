    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg,cache_dir='/DATA/DATA2/jzh/video_benchmark/cache', **kwargs)

            elif "qwen" in model_name.lower() or "quyen" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path,cache_dir='/fs-computility/ResearchEval/shared/hub')
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,attn_implementation=attn_implementation,cache_dir='/fs-computility/ResearchEval/shared/hub',**kwargs)

            elif "gemma" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, cache_dir='/DATA/DATA2/jzh/video_benchmark/cache', **kwargs)
            else:
                # try:
                from llava.model.language_model.llava_llama import LlavaConfig
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                                                              cache_dir='/DATA/DATA2/jzh/video_benchmark/cache',
