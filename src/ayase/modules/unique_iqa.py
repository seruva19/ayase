...
class UNIQUEModule(PipelineModule):
    ...
    def setup(self) -> None:
        try:
            ...
            self._model = pyiqa.create_metric("unique", device=device)
            try:
                self._device = next(self._model.parameters()).device
            except StopIteration:
                self._device = torch.device("cpu")
            self._ml_available = True
            logger.info("UNIQUE model loaded on %s", device)
        except (ImportError, Exception) as e:
            logger.warning("UNIQUE unavailable: %s", e)
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.unique_score = float(np.mean(scores))
        except Exception as e:
            logger.warning("UNIQUE processing failed: %s", e)
        return sample
...
class UNIQUECompatModule(UNIQUEModule):
