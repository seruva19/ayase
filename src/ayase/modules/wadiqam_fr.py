"""WaDIQaM-FR (Waterloo-IQA Deep Quality Metric, Full-Reference) module.

WaDIQaM-FR is the full-reference variant of WaDIQaM. It compares the
distorted image against a reference using deep features.
...
class WaDIQaMFRModule(PipelineModule):
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.wadiqam_fr = float(np.mean(scores))
        except Exception as e:
            logger.warning("WaDIQaM-FR processing failed: %s", e)
        return sample
