"""PreResQ-R1 — Fine-Grained Rank-and-Score (2025). presresq_score — higher = better"""
import logging, cv2, numpy as np
from ayase.models import Sample, QualityMetrics
from ayase.pipeline import PipelineModule
logger = logging.getLogger(__name__)
class PreResQModule(PipelineModule):
    name = "presresq"; description = "PreResQ-R1 rank+score VQA (2025)"; default_config = {"subsample": 8}
    def __init__(self, c=None):
        super().__init__(c); self.subsample = self.config.get("subsample", 8); self._backend = "heuristic"
    def setup(self):
        try: import presresq; self._model = presresq; self._backend = "native"; return
    ...
    def process(self, sample):
        try:
            ...
            sample.quality_metrics.presresq_score = float(np.clip(score, 0, 1))
        except Exception as e: logger.warning(f"PreResQ failed: {e}")
        return sample
    def _ex(self, sample):
        frames=[]
        if sample.is_video:
