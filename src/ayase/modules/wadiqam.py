...
class WaDIQaMModule(PipelineModule):
    ...
    def setup(self) -> None:
        try:
            ...
            self._model = pyiqa.create_metric("wadiqam_nr", device=device)
            try:
                self._device = next(self._model.parameters()).device
            except StopIteration:
                self._device = torch.device("cpu")
            self._ml_available = True
            logger.info("WaDIQaM-NR model loaded on %s", device)
        except (ImportError, Exception) as e:
            logger.warning("WaDIQaM-NR unavailable: %s", e)
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.wadiqam_score = float(np.mean(scores))
        except Exception as e:
            logger.warning("WaDIQaM processing failed: %s", e)
        return sample
