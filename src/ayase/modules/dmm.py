"""DMM (Detail Model Metric) module.

DMM is a full-reference image quality metric that focuses on detail
preservation. It is available in pyiqa as a full-reference metric.
...
class DMMModule(PipelineModule):
    ...
    def setup(self) -> None:
        try:
            ...
            self._model = pyiqa.create_metric("dmm", device=device)
            try:
                self._device = next(self._model.parameters()).device
            except StopIteration:
                self._device = torch.device("cpu")
            self._ml_available = True
            logger.info("DMM model loaded on %s", device)
        except (ImportError, Exception) as e:
            logger.warning("DMM unavailable: %s", e)
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.dmm = float(np.mean(scores))
        except Exception as e:
            logger.warning("DMM processing failed: %s", e)
        return sample
