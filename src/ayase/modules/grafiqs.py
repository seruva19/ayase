"""GraFIQs — Gradient-Based Face Quality (CVPRW 2024). grafiqs_score — higher = better"""
import logging, cv2, numpy as np
from ayase.models import Sample, QualityMetrics
from ayase.pipeline import PipelineModule
logger = logging.getLogger(__name__)
class GraFIQsModule(PipelineModule):
    name = "grafiqs"; description = "GraFIQs gradient face quality (CVPRW 2024)"; default_config = {"subsample": 4}
    def __init__(self, c=None):
        super().__init__(c); self.subsample = self.config.get("subsample", 4); self._face_cascade = None; self._backend = "heuristic"
    def setup(self):
        try:
            import grafiqs; self._model = grafiqs; self._backend = "native"; return
        except ImportError:
            pass
    ...
    def process(self, sample):
        try:
            if self._backend == "native" and self._model is not None:
                ...
                sample.quality_metrics.grafiqs_score = float(np.clip(score, 0, 1))
                return sample
            ...
            for f in frames:
                ...
                for (x,y,w,h) in faces:
                    ...
                    grad_mag = np.sqrt(cv2.Sobel(roi,cv2.CV_64F,1,0)**2+cv2.Sobel(roi,cv2.CV_64F,0,1)**2)
                    scores.append(min(grad_mag.mean()/25, 1))
            if scores:
                if sample.quality_metrics is None: sample.quality_metrics = QualityMetrics()
                sample.quality_metrics.grafiqs_score = float(np.clip(np.mean(scores), 0, 1))
        except Exception as e: logger.warning(f"GraFIQs failed: {e}")
        return sample
    def _ex(self, sample):
        frames=[]
        if sample.is_video:
