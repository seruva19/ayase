"""ProVQA — Progressive Blind 360 VQA. provqa_score — higher = better"""
import logging, cv2, numpy as np
from ayase.models import Sample, QualityMetrics
from ayase.pipeline import PipelineModule
logger = logging.getLogger(__name__)
class ProVQAModule(PipelineModule):
    name = "provqa"; description = "ProVQA progressive blind 360 VQA (2022)"; default_config = {"subsample": 8}
    def __init__(self, c=None):
        super().__init__(c); self.subsample = self.config.get("subsample", 8); self._backend = "heuristic"
    def setup(self):
        try: import provqa; self._model = provqa; self._backend = "native"; return
    ...
    def process(self, sample):
        try:
            ...
            # Progressive: pixel → frame → video
            pixel_q = float(np.mean([min(cv2.cvtColor(f,cv2.COLOR_BGR2GRAY).astype(float).std()/65,1) for f in frames]))
            frame_q = float(np.mean([min(cv2.Laplacian(cv2.cvtColor(f,cv2.COLOR_BGR2GRAY).astype(np.float64),cv2.CV_64F).var()/500,1) for f in frames]))
            video_q = 1.0
            if len(frames) > 1:
                ...
                video_q = 1.0/(1.0+np.var(d)*0.01)
            score = 0.25*pixel_q + 0.40*frame_q + 0.35*video_q
            if sample.quality_metrics is None: sample.quality_metrics = QualityMetrics()
            sample.quality_metrics.provqa_score = float(np.clip(score, 0, 1))
        except Exception as e: logger.warning(f"ProVQA failed: {e}")
        return sample
    def _ex(self, sample):
        frames=[]
        if sample.is_video:
