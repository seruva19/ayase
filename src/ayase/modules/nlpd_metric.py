...
class NLPDModule(PipelineModule):
    name = "nlpd"
    ...
    def setup(self) -> None:
        try:
            ...
            self._model = pyiqa.create_metric("nlpd", device=device)
            try:
                self._device = next(self._model.parameters()).device
            except StopIteration:
                self._device = torch.device("cpu")
            self._ml_available = True
            logger.info("NLPD model loaded on %s", device)
        except (ImportError, Exception) as e:
            logger.warning("NLPD unavailable: %s", e)
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.nlpd = float(np.mean(scores))
        except Exception as e:
            logger.warning("NLPD processing failed: %s", e)
        return sample
...
class NLPDCompatModule(NLPDModule):
