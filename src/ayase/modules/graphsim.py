...
class GraphSIMModule(ReferenceBasedModule):
    name = "graphsim"
    description = "GraphSIM graph gradient point cloud quality (2020)"
    metric_field = "graphsim_score"
    default_config = {}
    metric_groups = {
    ...
    def compute_reference_score(self, sample_path: Path, reference_path: Path) -> Optional[float]:
        ...
        try:
            ...
            _, nn1 = tree1.query(p1, k=k)
            _, nn2 = tree2.query(p2, k=k)
            if pc1.has_colors() and pc2.has_colors():
                c1, c2 = np.asarray(pc1.colors), np.asarray(pc2.colors)
                grad1 = np.std(
                    [c1[nn1[i]].std(axis=0) for i in range(min(1000, len(p1)))], axis=0
                ).mean()
                grad2 = np.std(
                    [c2[nn2[i]].std(axis=0) for i in range(min(1000, len(p2)))], axis=0
                ).mean()
                return float(1.0 / (1.0 + abs(grad1 - grad2) * 10))
            # Geometry-only gradient
            grad1 = np.std(
                [p1[nn1[i]].std(axis=0) for i in range(min(1000, len(p1)))], axis=0
            ).mean()
            grad2 = np.std(
                [p2[nn2[i]].std(axis=0) for i in range(min(1000, len(p2)))], axis=0
            ).mean()
            return float(1.0 / (1.0 + abs(grad1 - grad2) * 10))
        except ImportError:
