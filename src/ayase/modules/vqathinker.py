"""VQAThinker — Generalizable Explainable VQA via RL (2025).

RL-trained explainable video quality assessment model. This module requires
the released VQAThinker model; when that backend is not installed the metric
is left ``None`` — no CLIP zero-shot proxy or handcrafted approximation is
substituted for the published metric.

vqathinker_score — higher = better (0-1)
"""

import logging
from typing import Optional

from ayase.models import Sample, QualityMetrics
from ayase.pipeline import PipelineModule

logger = logging.getLogger(__name__)


class VQAThinkerModule(PipelineModule):
    name = "vqathinker"
    provisional = True  # no turnkey real backend in a standard install
    description = "VQAThinker RL-based explainable VQA (2025)"
    default_config = {
        "subsample": 8,
    }
    metric_groups = {
        "vqathinker_score": "nr_quality",
    }

    def __init__(self, config=None):
        super().__init__(config)
        self.subsample = self.config.get("subsample", 8)
        self._ml_available = False
        self._backend = None
        self._model = None

    def setup(self) -> None:
        if self.test_mode:
            return

        try:
            import vqathinker  # type: ignore  # official VQAThinker backend

            self._model = vqathinker
            self._ml_available = True
            self._backend = "vqathinker"
            logger.info("VQAThinker initialised (vqathinker backend)")
            return
        except ImportError:
            pass

        self._backend = "unavailable"
        self._ml_available = False
        logger.warning(
            "VQAThinker unavailable: the released VQAThinker model is not installed"
        )

    def process(self, sample: Sample) -> Sample:
        if sample.quality_metrics is None:
            sample.quality_metrics = QualityMetrics()
        if not self._ml_available or self._backend != "vqathinker":
            return sample

        try:
            predict = getattr(self._model, "predict", None)
            if predict is None:
                return sample
            score = predict(str(sample.path))
            if score is not None:
                sample.quality_metrics.vqathinker_score = float(score)
        except Exception as e:
            logger.warning("VQAThinker failed for %s: %s", sample.path, e)
        return sample
