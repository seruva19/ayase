...
class TReSModule(PipelineModule):
    ...
    def setup(self) -> None:
        try:
            ...
            self._model = pyiqa.create_metric("tres", device=device)
            self._ml_available = True
            logger.info("TReS model loaded on %s", device)
        except (ImportError, Exception) as e:
            logger.warning("TReS unavailable: %s", e)
    ...
    def process(self, sample: Sample) -> Sample:
        ...
        try:
            ...
            sample.quality_metrics.tres_score = float(np.mean(scores))
        except Exception as e:
            logger.warning("TReS processing failed: %s", e)
        return sample
