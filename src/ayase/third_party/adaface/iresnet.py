"""Vendored verbatim from CVLface ``models/iresnet/model.py`` (MIT, mk-minchul).

Source: https://huggingface.co/minchul/cvlface_adaface_ir101_webface12m/blob/main/models/iresnet/model.py
Upstream project: https://github.com/mk-minchul/CVLface (AdaFace, CVPR 2022)

Only modification: the ``fvcore``/``numpy`` imports and the ``__main__`` FLOP
counting block were dropped so the module has no dependency beyond torch.
"""
